...
//******************************************************************************
// *** OpenGL ES thread safety ***  iOS上的OpenGL ES不是线程安全的。
//
...
namespace {
    ...
    // Model scale factor
    const float kObjectScaleNormal = 0.9f; //正常大小
    const float kObjectScaleOffTargetTracking = 0.12f;
}
...
- (void)deleteFramebuffer; // 删除帧缓冲区
- (void)setFramebuffer; // 设置帧缓冲区
- (BOOL)presentFramebuffer; // 当前帧缓冲区
...
// 你必须实现这个方法，使视图的基础层是caeagllayer类
+ (Class)layerClass
{
...
- (void) renderFrameWithState:(const Vuforia::State&) state projectMatrix:(Vuforia::Matrix44F&) projectionMatrix {
    ...
    for (int i = 0; i < state.getNumTrackableResults(); ++i) {
        ...
        if (offTargetTrackingEnabled) {
            ...
            glDrawArrays(GL_TRIANGLES, 0, (int)bananaNumVerts);
        }
